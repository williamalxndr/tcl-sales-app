...
class LiveTests(SimpleTestCase):
    ...
    def test_method_error_is_json(self):
        response = self.client.post("/api/v1/health/live", {}, content_type="application/json")
        self.assertEqual(response.status_code, 405)
        self.assertEqual(response.json()["error"]["code"], "METHOD_NOT_ALLOWED")

    @override_settings(DEBUG=False)
    def test_unknown_business_routes_are_not_implemented(self):
        response = self.client.get("/api/v1/program-submissions")
    ...
    def test_cors_preflight_allows_configured_flutter_web_origin_only(self):
        response = self.client.options(
            "/api/v1/health/live", HTTP_ORIGIN="http://localhost:3000", HTTP_ACCESS_CONTROL_REQUEST_METHOD="GET"
        )
        self.assertEqual(response["Access-Control-Allow-Origin"], "http://localhost:3000")
        response = self.client.options(
            "/api/v1/health/live", HTTP_ORIGIN="https://untrusted.example", HTTP_ACCESS_CONTROL_REQUEST_METHOD="GET"
        )
...
class ReadyTests(TestCase):
    ...
    def test_database_outage_is_sanitized(self):
        with patch("apps.core.views.connection.cursor", side_effect=OperationalError("private database address")):
            response = self.client.get("/api/v1/health/ready")
